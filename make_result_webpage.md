...
<head>
  ...
  <!-- CDN Load Order: PapaParse for data parsing -->
...
## 8. Browser-Native Print Preview Overlay & Vector PDF Generation

Instead of using canvas-based libraries like `html2pdf.js` (which suffer from heavy download size, visual pixelation on high-res displays, and severe rendering errors on mobile), the scorecard is generated using **Browser-Native Printing** (`window.print()`). This ensures A4 vector-perfect PDFs directly generated by the browser's OS-level print engine.

### 8.1 The Mobile Layout Race Condition & The Preview Overlay Solution
Mobile browsers (like iOS Safari and Android Chrome) execute print dialogs asynchronously. Traditional single-page print workflows populate a hidden container, call `window.print()`, and immediately empty the container on the `onafterprint` event or a short timer. 
On mobile, `onafterprint` fires *during* preview generation. Emptying the DOM prematurely causes the print preview to corrupt, display blank pages, duplicate pages, or refresh.

To solve this, we implement a **Print Preview Overlay**:
1. **Interactive Screen Preview**: The scorecard is rendered as a fullscreen dark overlay on screen, centered to resemble a physical A4 sheet.
2. **Floating Controls**: Action buttons ("❌ Close Preview" and "💾 Print / Save PDF") are displayed at the top of the viewport.
3. **No Automatic Destruct**: We remove `onafterprint` hooks. The scorecard remains in the DOM while printing, allowing mobile rendering engines a stable document layout. The user closes the preview manually when done.

### 8.2 CSS Styling Implementation

#### Screen Styling
```css
/* Print scorecard overlay on screen - hidden by default */
#print-scorecard {
  display: none;
}

/* Screen overlay backdrop */
#print-scorecard.active {
  display: flex;
  position: fixed;
  top: 0;
  left: 0;
...
.print-control-btn.close-btn {
  background: #F44336;
}
.print-control-btn.close-btn:hover {
  background: #d32f2f;
}
.print-control-btn.print-btn {
  background: var(--saffron);
}
.print-control-btn.print-btn:hover {
  background: var(--saffron-light);
}

/* A4 Scorecard card styling on screen */
#print-scorecard .sc-card {
  background: #fff;
...
@media print {
  ...
  body > *:not(#print-scorecard) {
    display: none !important;
  }
  
  /* Reset overlay wrapping for raw body print */
  #print-scorecard {
  ...
  /* Hide floating preview buttons */
  .print-controls {
    display: none !important;
  }
  
  /* Flatten paper sheet layout */
  #print-scorecard .sc-card {
...
```javascript
/**
 * Renders scorecard HTML inside a fullscreen preview modal,
 * overlays controls, and invokes browser print dialog.
...
function downloadCandidatePDF(index) {
  ...
  const tsp      = candidate['TSP'] === 'YES'  ? 'YES (TSP)' : 'NO (Non-TSP)';
  const subCat   = candidate['Sub Category']   || 'None';

  const getMarks    = (d) => parseFloat(d.Marks || d.marks || 0);
  const getRollNo   = (d) => d['Roll No'] || d['roll no'] || d.RollNo || '';
  const getCategory = (d) => (d.Category || d.category || d.Cat || d.cat || 'GEN').toUpperCase();

  const sameCategory    = allData.filter(d => getCategory(d) === category.toUpperCase()).sort((a,b) => getMarks(a) - getMarks(b));
  const catPosition     = sameCategory.findIndex(d => getRollNo(d) === roll) + 1;
  ...
  const catAvg          = (sameCategory.reduce((sum, d) => sum + getMarks(d), 0) / totalInCat).toFixed(2);
  const catTop          = sameCategory[sameCategory.length - 1];
  const catTopMarks     = getMarks(catTop).toFixed(2);
  const diff            = parseFloat(marks) - parseFloat(catAvg);
  const aboveBelow      = diff >= 0 ? '+' + diff.toFixed(2) + ' marks above avg' : diff.toFixed(2) + ' marks below avg';
  const aboveBelowColor = diff >= 0 ? '#138808' : '#F44336';

  const container = document.getElementById('print-scorecard');
  container.innerHTML = `
    <div class="print-controls">
      <button class="print-control-btn close-btn" onclick="closePrintPreview()">❌ Close Preview</button>
      <button class="print-control-btn print-btn" onclick="window.print()">💾 Print / Save PDF</button>
    </div>
    <a href="https://pavnxet.github.io/3rd-grade-result/" target="_blank">
      <div class="sc-card">
        <!-- Header -->
        <div class="sc-header">
          <div class="sc-notice">💡 Click anywhere on this document to verify this result online</div>
          <h2 class="sc-title">Rajasthan Staff Selection Board (RSSB)</h2>
          <h3 class="sc-sub">Candidate Performance Report Card</h3>
          <p class="sc-meta">Subject: ${subjectName} &middot; Level 2 Exam</p>
        </div>
        
        <!-- Candidate details table -->
        <div class="sc-section-title">Candidate Information</div>
        <table class="sc-info-table">
          <tr>
            <td class="sc-bold">Roll Number:</td><td>${roll}</td>
            <td class="sc-bold">Merit Rank:</td><td class="sc-orange">#${merit}</td>
          </tr>
          <tr><td class="sc-bold">Candidate Name:</td><td colspan="3">${name}</td></tr>
          <tr><td class="sc-bold">Father's Name:</td><td colspan="3">${father}</td></tr>
          <tr><td class="sc-bold">Mother's Name:</td><td colspan="3">${mother}</td></tr>
          <tr>
            <td class="sc-bold">Gender:</td><td>${gender}</td>
            <td class="sc-bold">Category:</td><td>${category}</td>
          </tr>
          <tr>
            <td class="sc-bold">Sub Category:</td><td>${subCat}</td>
            <td class="sc-bold">TSP Candidate:</td><td>${tsp}</td>
          </tr>
        </table>
        ...
        <div class="sc-metrics">
          <div class="sc-metric-box">
            <div class="sc-metric-lbl">Obtained Marks</div>
            <div class="sc-metric-val sc-green">${marks}</div>
            <div class="sc-metric-sub">out of 300 marks</div>
          </div>
          <div class="sc-metric-box">
            <div class="sc-metric-lbl">Category Rank (${category})</div>
            <div class="sc-metric-val sc-orange">#${catRank}</div>
            <div class="sc-metric-sub">out of ${totalInCat} candidates</div>
          </div>
          <div class="sc-metric-box">
            <div class="sc-metric-lbl">Performance Percentile</div>
            <div class="sc-metric-val sc-dark">${percentile}%</div>
            <div class="sc-metric-sub">Top ${(100 - parseFloat(percentile)).toFixed(1)}% of all candidates</div>
          </div>
        </div>

        <!-- Benchmarks -->
        <div class="sc-section-title">Category Benchmark Comparison</div>
        <table class="sc-bench">
          <thead>
            <tr>
              <th>Metric</th>
              <th>Marks</th>
              <th>Status / Difference</th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <td class="sc-bold">Category Highest:</td>
              <td class="sc-bold sc-red">${catTopMarks}</td>
              <td class="sc-red" style="font-size:12px">-${(parseFloat(catTopMarks) - parseFloat(marks)).toFixed(2)} marks behind top</td>
            </tr>
            <tr>
              <td class="sc-bold">Candidate Marks:</td>
              <td class="sc-bold sc-green">${marks}</td>
              <td class="sc-bold sc-green" style="font-size:12px">Reference Base</td>
            </tr>
            <tr>
              <td class="sc-bold">Category Average:</td>
              <td>${catAvg}</td>
              <td style="color:${aboveBelowColor}; font-weight:bold; font-size:12px">${aboveBelow}</td>
            </tr>
          </tbody>
        </table>
        ...
        <!-- Footer -->
        <div class="sc-footer">
          <div>Generated via RSSB Merit Search Engine &middot; pavnxet</div>
  ...
  // Close when clicking outside card on background overlay
  container.onclick = (e) => {
...
/**
 * Hides and cleans up the active print preview container.
 */
