...
*   **📄 Clickable PDF Scorecard Export (Native Print Preview Overlay)**: Save a candidate's detailed report card as a beautifully styled vector PDF. Using a fullscreen print preview overlay combined with browser-native printing (`window.print()`), this approach bypasses heavy, pixelated canvas libraries (like `html2pdf.js`) and works flawlessly on both desktop and mobile devices (avoiding layout race conditions). The entire document page acts as an embedded hyperlink—clicking or touching it anywhere redirects the reader back to the live website (`https://pavnxet.github.io/3rd-grade-result/`) for instant online verification.
